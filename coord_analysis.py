...
if __name__ == "__main__":
    ...
    data = data_read.sample(frac=0.2)
    ...
    max_depths = np.append(np.append(np.arange(1,3,1), np.arange(25, 300, 25)), [None])
    # max_depths = [1]
    ...
    for max_depth in max_depths:
        ...
        for train_index, test_index in kf:
            ...
            for i in range(len(rides_ls)):
                ...
                #preds = np.concatenate(([X_ls[f].iloc[i] for f in predictors],taxiId[i]),axis=1)
            ...
            knn_25 = DecisionTreeRegressor(max_depth=max_depth)
            knn_50 = DecisionTreeRegressor(max_depth=max_depth)
            knn_100 = DecisionTreeRegressor(max_depth=max_depth)
            knn_plus = DecisionTreeRegressor(max_depth=max_depth)
        ...
        print("DecisionTree with max_depth = {}".format(max_depth))
